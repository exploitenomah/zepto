...
export function OptionItem({ option, search }: {
  ...
  return (
    <span className="flex gap-3 items-center">
      ...
      <span dangerouslySetInnerHTML={{ __html: highlightMatch(option.email, search as string) }} className="text-gray-600 text-sm" />
    </span>
  )
}

export function SelectedChipItem({ option }: {
  option: UserOption,
}) {
  return (
    <span className="flex gap-3 items-center">
      <img width={30} height={30} src={option.image} alt={`${option.firstName} ${option.lastName}`}
        className="rounded-full bg-green-300 border shadow-sm" />
